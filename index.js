...
program
  ...
  .option('--fileType [value]', 'It supports TS and JS')
  .option('-C, --includeDeprecatedFields [value]', 'Flag to include deprecated fields (The default is to exclude)')
  .parse(process.argv);

const { schemaFilePath, destDirPath, depthLimit = 100, includeDeprecatedFields = false, fileType = 'js' } = program;
...
const generateFile = (obj, description) => {
  ...
  Object.keys(obj).forEach((type) => {
    ...
    if (includeDeprecatedFields || !field.isDeprecated) {
      ...
      query = `import { gql } from '@apollo/client';\n\nexport const ${type} = gql\`
      ${description.toLowerCase()} ${type}${varsToTypesStr ? `(${varsToTypesStr})` : ''}{\n${query}\n}\`;`;

      fs.writeFileSync(path.join(writeFolder, `./${type}.${fileType}`), query);
      indexJs += `export * from './${type}'\n`;
    }
  });

  fs.writeFileSync(path.join(writeFolder, `index.${fileType}`), indexJs);
...
fs.writeFileSync(path.join(destDirPath, `index.${fileType}`), indexJsExportAll);
